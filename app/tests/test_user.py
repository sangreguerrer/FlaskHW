...
from .constants import DEFAULT_USER_NAME, DEFAULT_USER_PASSWORD, NEW_USER_NAME, DEFAULT_USER_EMAIL


class TestUser:
    def test_create_user(self, client):
        response = client.create_user("test_create_user", DEFAULT_USER_EMAIL, DEFAULT_USER_PASSWORD)
        assert response.id is not None

    def test_create_user_with_same_name(self, client):
        client.create_user("test_create_user_with_same_name", DEFAULT_USER_EMAIL, DEFAULT_USER_PASSWORD)
    ...
    def test_create_user_with_simple_password(self, client):
        with pytest.raises(HttpError) as excinfo:
            client.create_user("test_create_user_with_simple_password", DEFAULT_USER_EMAIL, "123")
        assert excinfo.value.status_code == 400

    def test_auth_user(self, client):
        client.create_user("test_auth_user", DEFAULT_USER_EMAIL, DEFAULT_USER_PASSWORD)
        response = client.login("test_auth_user", DEFAULT_USER_PASSWORD)
        assert response.token is not None

    def test_auth_user_with_wrong_password(self, client):
        client.create_user("test_auth_user_with_wrong_password", DEFAULT_USER_EMAIL, DEFAULT_USER_PASSWORD)
        with pytest.raises(HttpError) as excinfo:
            client.login("test_auth_user_with_wrong_password", "wrong_password")
        assert excinfo.value.status_code == 401

    def test_auth_user_with_wrong_name(self, client):
        client.create_user("test_auth_user_with_wrong_name", DEFAULT_USER_EMAIL, DEFAULT_USER_PASSWORD)
        with pytest.raises(HttpError) as excinfo:
    ...
    def test_update_user_with_same_name(self, new_user_client):
        ...
        except pytest.raises(HttpError) as excinfo:
            assert excinfo.value.status_code == 409
            user = new_user_client.get_user()
    ...
    def test_delete_user(self, client):
        client.create_user("test_delete_user",DEFAULT_USER_EMAIL, DEFAULT_USER_PASSWORD)
